...
async function UpdateTraining (req) {
...
async function generatePreImages(req, prompts) {
	...
	prompts.forEach(async (prompt, index) => {
		...
		axios.post('http://localhost:3000/api/v1/PostInference', data).then((res) => {
		}).catch((err) => {
			console.log(err);
		})
	})
  ...
  async function FinishTraining (req) {
  
	const user_id = req.user._id;
	const token_id = req.body.token_id;
  
	const Myprompts = [`photo of ${token_id} looking left, classic, old styles, insane details`, `photo of ${token_id} as astronaut, High detail RAW color art, High Detail, Sharp focus, dramatic, photorealistic painting art by midjourney and greg rutkowski, bokeh on background`, `photo of ${token_id} as Military Leader, High detail RAW color art, High Detail, Sharp focus, dramatic, photorealistic painting art by midjourney and greg rutkowski, bokeh on background`, `High detail RAW color art, animation, ${token_id} as Thorin Oakenshield from LOTR, (inside the mountain dwarven halls), ((Dwarf king)), ((((crown on head whit Arkenstone)))) Black leather armor, ((dirty Black long curly hair)) huge black wolf fur collar ((against the background of gold placers smaug dragon)) Atey Ghailan, by Jeremy Mann, Greg Manchess, Antonio Moro, trending on ArtStation, trending on CGSociety, Intricate, High Detail, Sharp focus, dramatic, photorealistic painting art by midjourney and greg rutkowski, bokeh on background`, `photo of ${token_id} as astronaut, High detail RAW color art, High Detail, Sharp focus, dramatic, photorealistic painting art by midjourney and greg rutkowski, bokeh on background`, `High detail RAW color art, animation, ${token_id} as Bilbo Baggins from LOTR, (hobbit of the Shire), ((against the background of gold placers smaug dragon)) Atey Ghailan, by Jeremy Mann, Greg Manchess, Antonio Moro, trending on ArtStation, trending on CGSociety, Intricate, High Detail, Sharp focus, dramatic, photorealistic painting art by midjourney and greg rutkowski, bokeh on background`, `photo of ${token_id} as Military Leader, High detail RAW color art, High Detail, Sharp focus, dramatic, photorealistic painting art by midjourney and greg rutkowski, bokeh on background`, `photo of ${token_id} as Flight Attendant, High detail RAW color art, High Detail, Sharp focus, dramatic, photorealistic painting art by midjourney and greg rutkowski, bokeh on background`, `portrait of ${token_id}, looking to camera, insane details, city, 4k`, `selfie of ${token_id}, insane details, 4k, realistic`, `Old ${token_id} with wrinkles face looking left, (stylish hairstyle:1.3), gray hair, clear facial features, piercing gaze, (military clothes:1.2), intricate details), epic, High Detail, Sharp focus, dramatic, photorealistic painting art by midjourney and greg rutkowski`];
...
function makeApiRequest(req) {
  ...
  return new Promise((resolve, reject) => {
	...
	const payload = {
	  ...
	  resolve(
		banana.start(BANANA_API, BANANA_MODEL_KEY, payload).then((training_id) => {
			const ID = training_id;
			UpdateTraining(req).then(() => {
				setTimeout(() => {
					function CheckStatus() {
						banana.check(BANANA_API, ID).then((status) => {
							// console.log(status);
							if (status.message == "running" || status.message == "pending" || status.message == "queued")
								setTimeout(CheckStatus, 60000);
							else if (status.message == "success"){
								FinishTraining(req).then((res) => {
